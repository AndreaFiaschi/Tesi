...
Ad esempio, la scelta di alimentare il chip separando la parte analogica da quella digitale deriva dalla necessità del sistema (???OK???) di proteggere la prima, più sensibile, dal rumore della seconda.
...
\item \textbf{meta$\_$tune$\_$threshold$\_$simple} Scan delle soglie per ogni possibile TDAC al fine di ottenere il valore ottimale per ogni pixel che consente di avvicinarsi alla soglia voluta.
...
!!! IO QUI RIPORTEREI COME E' FATTO IL FIT !!!
...
Le misure riportate fanno riferimento solo al \textit{front end} lineare, in quanto al momento della presa dati, era quello su cui era possibile lavorare meglio, sia a livello di ottimizzazione dei vari parametri, sia a livello di software (??? NON BELLISSIMO, COSA SI VUOLE DIRE ???).
