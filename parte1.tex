...
Il progetto del regolatore SLDO ha subito nel tempo modifiche e migliorie, fino ad arrivare all'attuale versione in tecnologia CMOS a $65\nm$, capace di lavorare con una corrente massima di 2A.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.5\textwidth]{Immagini/CaratteristicaSLDO.png}
\caption{Caratteristica corrente-tensione per uno ShuntLDO che mostra $\mathrm{V_{in}}$ (curva) e $\mathrm{V_{out}}$ (curva) in funzione di $\mathrm{I_{in}}$; la regolazione di  $\mathrm{V_{out}}$ al valore di $$ inizia per $\mathrm{I_{in}}\sim$. Al di sopra di questo valore   $\mathrm{V_{in}}$ ha una pendenza pari a $\sim \frac{R3}{k}$.}
\label{SLDO5A}
\end{figure}
...
La caratteristica resistiva, inoltre, fa s\`i che due ShuntLDO in parallelo offriranno una resistenza equivalente pari al parallelo delle resistenze equivalenti dei singoli dispositivi superando le problematiche descritte sopra che affliggono un tradizionale circuito shunt basato su diodo Zener.
