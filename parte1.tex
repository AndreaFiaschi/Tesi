...
Inoltre, le variazioni dei consumi sono molto veloci e gli alimentatori di \textit{back-end} non sono in grado di compensarle in modo rapido: questi saranno, infatti, posizionati lontano dall'esperimento e i cavi introdurrano un ritardo proporzionale alla lunghezza degli stessi.
...
Con questo tipo di configurazione la corrente erogata dagli alimentatori di \textit{back-end} deve essere maggiore o uguale a quella necessaria all'elemento con consumo più alto.
...
%capitolo
\section{Prova}
\par \begin{center} !!! Prima parte già presente nell'introduzione !!! \end{center} \par
Riassumendo quanto già detto rispetto ad uno schema di alimentazione parallelo o diretto, dove ogni modulo è alimentato separatamente (e quindi ha un suo set di cavi!!!!!), l'utilizzo di una alimentazione seriale permette una notevole riduzione dei costi e del numero di cavi necessari: per n moduli sono necessari solo due cavi ed un unico generatore. 
...
Oltre a ciò, l'alimentazione seriale ha altri benefici rispetto a sistemi di alimentazione basati sulla tensione. 
...
Inoltre, l'utilizzo dello SLDO consente di regolare la tensione in prossimità del carico, senza necessità di altri cavi per misure delle tensioni locali (alimentazioni dirette), e di rispondere in modo più celere a cambiamenti del carico in corrente. La corrente che scorre nei moduli è costante e perciò non ci sono transienti in ingresso al regolatore. 
D'altra parte nell'alimentazione seriale la dissipazione di potenza dei moduli è in media più alta che in configurazioni di alimentazione diretta o parallela. 
Questo maggiore consumo di potenza è però compensato dalla notevole riduzione di quella dissipata nei cavi. 

Inoltre, dal momento che i moduli sono connessi in serie, l'alimentazione seriale comporta dei rischi. 
...
In una alimentazione seriale i moduli non possono essere spenti/accesi individualmente, e per di più tutti i moduli hanno tensioni di riferimento differenti tra di loro e rispetto al sistema di lettura e al generatore. 
...
Come già detto in uno schema di alimentazione parallelo i regolatori devono essere in grado di lavorare in parallelo, generare differenti tensioni dalle correnti in ingresso e tenere costante il consumo di corrente tramite lo shunt.
\begin{figure}
\centering
...
\label{zener}
\end{figure}
I regolatori con shunt sono utilizzati per convertire la corrente in una tensione costante. Il principio di lavoro è simile ad un diodo zener contropolarizzato, figura \ref{zener}. 
...
Questo fa si che, al momento in cui ho più elementi in parallelo, ci sia un diodo zener che va in \textit{break down} prima degli altri assorbendo tutta la corrente che dovrebbe distribuirsi nel parallelo rischiando di bruciare. 
