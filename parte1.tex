...
La tensione di uscita del generatore è dunque $\mathrm{V=I \cdot 2 \Omega +12.6V=31.8V}$\footnote{Circa il $60 \%$ della potenza è dissipata sui cavi, questo fatto potrebbe sembrare un pessimo traguardo, in realtà è un miglioramento rispetto alla situazione attuale, nella quale l'alimentazione è in parallelo.}. 
...
\caption{Schema di una catena seriale con moduli, ognuno formato da 4 chip. Il chip in rosso si riferisce all'ipotesi di un faure.}
\label{MCM}
\end{figure}
...
All'interno del chip RD53A la gestione dell'alimentazione è assegnata al regolatore SLDO, il cui studio è oggetto di questa tesi, partiamo dunque con il descrivere il circuito partendo dai primi prototipi, fino ad arrivare alla versione attualmente presente in RD53A.
...
%\label{PCB2A}
%\end{figure}
Rispetto a quanto visto in precedenza, nella versione da 2 A è possibile gestire anche l'offset attraverso un potenziometro RP2 che va ad agire sulla tensione in uscita generata dal bandgap, la stessa che viene utilizzata per generare $\mathrm{V_{ref}}$ regolando un secondo potenziometro RP1. 
