...
\end{equation}
dove B è il campo magnetico, r il raggio di curvatura e P l'impulso della particella. Dati i parametri di LHC (r$\sim4 \cdot 10^3\m$ e P$\simeq 7\TeVc$) si ottiene un campo B che in media è pari a $\sim 6{\mathrm T}$. In realt\`a i dipoli impiegati hanno un campo magnetico B di $\sim 8.3\T$, dal momento che occupano circa i 2/3 dell'anello e localmente il raggio di curvatura \`e pi\`u stretto. Per ottenere un campo di tale intensit\`a è stato necessario sviluppare dipoli magnetici superconduttori, il che ha rappresentato un'importante sfida tecnologica per la progettazione di LHC. Gran parte dell'anello, infatti, \`e mantenuto a temperature criogeniche di $\sim 2\K$ grazie ad un complesso sistema di raffreddamento.
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{Immagini/int_lumi_cumulative_pp_2.png}
